...
onto a sequence of figures. When figure order must mirror the manuscript's
claim escalation, run it alongside the `results-section-revision` skill, which
owns the Results-side argument progression.

This file is the panel-level evidence layer. The figure-level contract (core
conclusion, archetype, backend, export) stays in `references/figure-contract.md`;
the pre-delivery checklist stays in `references/qa-contract.md`. Do not restate
either here.
...
This is an archetype, not a mandatory six-figure template. Audit the escalation
against the Results section itself; the `results-section-revision` skill carries
that audit. If two figures can both be summarized as `X performs well`, merge
them, demote the weaker evidence, or revise the later figure around a deeper
discriminator.

## Assign visual hierarchy from evidence hierarchy

The hero-panel rule, the one-colour-per-condition rule and the
do-not-force-equal-panels rule are already stated in `static/core/stance.md` and
`references/figure-contract.md`. Apply them from there. This layer adds only
what follows from the evidence roles above:
...
- Can the caption explain what is shown and how to read it without replaying
  the full Results argument? Caption wording rules live in
  `references/figure-legend-conventions.md`.
- At final physical size, are dependencies, shared encodings, uncertainty and
  panel transitions still legible? Size, type and export checks live in
