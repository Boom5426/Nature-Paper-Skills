...
The gate has three outcomes, never two. ``PASS`` and ``FIX BEFORE DELIVERY``
say the geometry was measured. ``NOT AUDITABLE`` and the ``panel-not-auditable``
WARN say the geometry could not be measured, which is not the same thing as
being correct: a hand-built ``fig.add_axes()`` layout carries no grid metadata,
so the caller must declare ``row_groups``/``column_groups`` (or record an
exemption) before the layout can be checked at all.
"""
...
# Panel letters are typeset in either case: kernel.panel_letter(case="upper")
# emits "A", the Nature house default emits "a". Matching only [a-z] made every
# uppercase figure report "no panel labels" while still returning a green PASS.
PANEL_LETTER_PATTERN = re.compile(r"[A-Za-z]")

GROUPS_REQUIRED_MESSAGE = (
    "declare row_groups=[[...]] and column_groups=[[...]] explicitly "
    "(matplotlib_layout_manifest/require_matplotlib_panel_alignment accept both), "
    "or record an exemption with a reason for panels that are deliberately "
    "outside every comparable group"
)
...
def is_panel_letter(text: Any) -> bool:
    """Return True when ``text`` is a single panel letter in either case."""
    return bool(PANEL_LETTER_PATTERN.fullmatch(str(text)))


def _atomic_write_text(path: Path, content: str) -> None:
    """Write ``content`` to ``path`` atomically, with umask-derived permissions.

    QA reports and diagnostic overlays are ordinary build artifacts that the
    author is expected to read, attach to a revision, and hand to a co-author.
    ``tempfile.mkstemp`` would force 0600 on them regardless of the process
    umask, so the file is created with ``O_CREAT | O_EXCL`` and mode 0666, which
    the kernel masks with the caller's umask exactly as a plain ``open()`` would.
    """
...
def _validated_layout(manifest: Mapping[str, Any]) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    for group in [*row_groups, *column_groups, *boundary_groups]:
        grouped_ids.update(group["panels"])
    fully_exempt = {
        panel_id
        for panel_id in panel_ids
        for exemption in exemptions
        if panel_id in exemption["panels"]
        and ("all" in exemption["checks"] or {"row", "column"} <= set(exemption["checks"]))
    }
    ungrouped = [
        panel["id"]
        for panel in panels
        if panel["id"] not in grouped_ids and panel["id"] not in fully_exempt
    ]
    without_grid_metadata = [
        panel["id"]
        for panel in panels
        if any(
            panel.get(field) is None
            for field in ("grid_id", "row_start", "row_stop", "col_start", "col_stop")
        )
    ]
    ...
    return {
        ...
        "panels_without_grid_metadata": without_grid_metadata,
        "panel_label_detection": dict(manifest.get("panel_label_detection") or {}),
    }, []
...
def _not_auditable_errors(layout: Mapping[str, Any]) -> list[str]:
    """Explain why nothing could be compared, and what the caller must supply."""
    total = len(layout.get("panels", []))
    missing = list(layout.get("panels_without_grid_metadata") or [])
    if missing:
        return [
            f"No comparable row, column or shared-boundary groups could be inferred: "
            f"{len(missing)} of {total} panels carry no grid metadata "
            f"(matplotlib SubplotSpec, patchwork/gtable cell) "
            f"[{', '.join(missing)}]. A hand-built fig.add_axes() layout cannot be "
            f"audited from geometry alone, because nothing in it says which panels "
            f"are meant to line up. This is NOT a pass: to audit the figure, "
            f"{GROUPS_REQUIRED_MESSAGE}."
        ]
    return [
        "No comparable row, column or shared-boundary groups were declared or "
        f"inferred, so no alignment was checked. This is NOT a pass: to audit the "
        f"figure, {GROUPS_REQUIRED_MESSAGE}."
    ]
...
def audit_layout_manifest(
    ...
    *,
    ...
    detection = layout.get("panel_label_detection") or {}
    unlabelled = [
        panel["id"]
        for panel in layout["panels"]
        if not _is_exempt(layout, panel["id"], "panel-label")
    ]
    if detection.get("attempted") and not int(detection.get("detected", 0) or 0) and unlabelled:
        findings.append(
            {
                "severity": "WARN",
                "kind": "panel-label-not-detected",
                "group": "figure",
                "panels": unlabelled,
                "message": (
                    "Panel-label alignment was NOT checked: the backend scanned "
                    f"{int(detection.get('panels', 0) or 0)} panels and detected no panel letter. "
                    "A letter is recognised when it is a single [A-Za-z] character, "
                    "drawn with a bold font weight, anchored near the top-left of its axes. "
                    "Draw the letters that way, pass panel_label_anchor_pt explicitly, or record a "
                    "panel-label exemption with a reason"
                ),
            }
        )

    ungrouped = list(layout.get("ungrouped_panels") or [])
    if ungrouped and comparisons > 0:
        findings.append(
            {
                "severity": "WARN",
                "kind": "panel-not-auditable",
                "group": "figure",
                "panels": ungrouped,
                "message": (
                    "Alignment was NOT checked for these panels: they belong to no "
                    "declared or inferable comparable group, so nothing was compared "
                    f"against them. To audit them, {GROUPS_REQUIRED_MESSAGE}"
                ),
            }
        )
    ...
    return {
        ...
        "layout": layout,
        "findings": findings,
        **({"errors": _not_auditable_errors(layout)} if not auditable else {}),
    }
...
def _matplotlib_panel_label_anchor(ax: Any) -> tuple[str, list[float]] | None:
    for artist in ax.texts:
        ...
        if not is_panel_letter(label) or not _font_is_bold(artist.get_fontweight()):
            continue
...
def matplotlib_layout_manifest(
    ...
    *,
    ...
    label_anchors = [_matplotlib_panel_label_anchor(axis) for axis in selected]
    detected_letters = [None if anchor is None else anchor[0] for anchor in label_anchors]
    ...
    if isinstance(panel_ids, Mapping):
        ...
        # Prefer an explicit gid/label, then the panel letter the reader actually
        # sees, and only then a positional fallback. Naming a finding "b" when the
        # figure prints "B" over a different panel sends the author to the wrong
        # place, so the detected letters are used only when they are unambiguous.
        letters_usable = (
            all(letter is not None for letter in detected_letters)
            and len(set(detected_letters)) == len(detected_letters)
        )
        ids = []
        for index, axis in enumerate(selected):
            ...
            if candidate and not candidate.startswith("<"):
                ...
                ids.append(str(detected_letters[index]))
            else:
                ids.append(_alphabetic_id(index))

    grid_ids: dict[int, str] = {}
    panels: list[dict[str, Any]] = []
    for axis, panel_id, label_anchor in zip(selected, ids, label_anchors):
    ...
    manifest: dict[str, Any] = {
        ...
        # Records that the label detector ran, so a figure whose letters were
        # never found reports that the panel-label check did not run instead of
        # quietly reporting a clean layout.
        "panel_label_detection": {
            "attempted": True,
            "panels": len(selected),
            "detected": sum(anchor is not None for anchor in label_anchors),
        },
    }
...
def run_self_tests() -> None:
    ...
    for candidate in ("a", "Z", "q"):
        if not is_panel_letter(candidate):
            raise AssertionError(f"panel letter {candidate!r} was not recognised")
    for candidate in ("ab", "1", "", "a."):
        if is_panel_letter(candidate):
            raise AssertionError(f"{candidate!r} must not be treated as a panel letter")

    # A layout with no grid metadata cannot be audited, and must say so.
    ungrouped_only = {
        "schema_version": 1,
        "backend": "self-test",
        "figure": {"width_pt": 300, "height_pt": 200},
        "panels": [
            ...
            {"id": "b", "bbox_pt": [170, 105, 280, 175]},
        ],
    }
    ungrouped_report = audit_layout_manifest(ungrouped_only)
    if ungrouped_report["verdict"] != "NOT AUDITABLE" or exit_code(ungrouped_report) != EXIT_NOT_AUDITABLE:
        raise AssertionError("grid-metadata-free self-test layout was not reported unauditable")
    if not any("row_groups" in error for error in ungrouped_report.get("errors", [])):
        raise AssertionError("unauditable self-test layout did not name the required remedy")

    # A mixed layout must never report PASS for the panels it could not compare.
    mixed = json.loads(json.dumps(aligned))
    mixed["panels"].append({"id": "e", "bbox_pt": [285, 5, 295, 15]})
    mixed_report = audit_layout_manifest(mixed)
    if mixed_report["verdict"] != "REVIEW REQUIRED" or not any(
        finding.get("kind") == "panel-not-auditable"
        for finding in mixed_report.get("findings", [])
    ):
        raise AssertionError("mixed self-test layout hid an uncompared panel behind a pass")

    # Reports follow the umask instead of being forced to 0600.
    previous_umask = os.umask(0o022)
    try:
        with tempfile.TemporaryDirectory() as directory:
            report_path = Path(directory) / "alignment.json"
            write_json_report(audit_layout_manifest(aligned), report_path)
            mode = report_path.stat().st_mode & 0o777
            if mode != 0o644:
                raise AssertionError(f"report mode {oct(mode)} does not follow umask 022")
    finally:
